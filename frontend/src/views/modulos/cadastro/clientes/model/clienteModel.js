class ClienteModel {
    constructor(data) {
        (this.id = Number(data.id) || undefined),
            ...
            (this.cpf = data.cpf ? String(data.cpf).replaceAll('.', '').replaceAll('-', '') : ''),
            (this.rg = data.rg ? String(data.rg).replaceAll('.', '').replaceAll('-', '') : ''),
            (this.data_nascimento = data.data_nascimento ? new Date(data.data_nascimento) : null),
            (this.cnpj = data.cnpj ? String(data.cnpj) : ''),
            (this.telefone = data.telefone ? String(data.telefone).replaceAll('+', '').replaceAll('(', '').replaceAll(')', '').replaceAll('-', '') : ''),
