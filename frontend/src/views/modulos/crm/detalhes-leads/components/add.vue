<template>
  ...
  <div class="modal-footer">
    <button type="button" class="btn btn-danger" @click="back(get_task)">Voltar</button>
    <button type="button" class="btn btn-success" @click="saveSubTask">Salvar</button>
  </div>
...
export default {
  ...
  methods: {
    ...
    async saveTask() {
      ...
      if (this.params.id) {
        //update task
        taskUpdate['processoId'] = this.project.id;

        let taskUpdate = new TaskModel(this.params);
        taskUpdate['id'] = this.params.id;
        ...
        this.$router.push(`/crm/fluxo/${this.project.id}`);



      } else {
        this.params['projetoId'] = this.project.id;
